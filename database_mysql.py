"""Implementacja MySQL — przechowywanie danych na serwerze MySQL."""
import logging
...
class MySQLDatabase:
    ...
    def __init__(self):
        ...
        logger.info("MySQL: %s@%s/%s", self.config['user'], self.config['host'], self.config['database'])
        self.init_database()
    ...
    def init_database(self):
        ...
        try:
            ...
            conn.commit()
            logger.info("Tabele MySQL gotowe")

        except Exception as e:
            logger.error("Błąd inicjalizacji bazy: %s", e)
            conn.rollback()
            raise
        finally:
            cursor.close()
    ...
    def create_user(
        ...
        try:
            ...
            logger.error("Tworzenie użytkownika nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def block_user(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error("Blokada użytkownika nie powiodła się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def unblock_user(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error("Odblokowanie nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def add_balance(self, user_id: int, amount: int) -> bool:
        ...
        try:
            ...
            logger.error("Dodanie punktów nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def deduct_balance(self, user_id: int, amount: int) -> bool:
        ...
        try:
            ...
            logger.error("Pobranie punktów nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def can_checkin(self, user_id: int) -> bool:
        """Czy można się zalogować dziś (/qd)."""
        user = self.get_user(user_id)
        if not user:
            return False
    ...
    def checkin(self, user_id: int) -> bool:
        """Codzienne logowanie — jedna próba dziennie (atomowo w SQL)."""
        conn = self.get_connection()
        cursor = conn.cursor()

        try:
            # Aktualizacja tylko gdy brak logowania lub data < dziś
            cursor.execute(
                """
                UPDATE users
            ...
            logger.error("Logowanie dzienne nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def add_verification(
        ...
        try:
            ...
            logger.error("Zapis weryfikacji nie powiódł się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def create_card_key(
        ...
        try:
            ...
            logger.error("Kod już istnieje: %s", key_code)
            conn.rollback()
            return False
        except Exception as e:
            logger.error("Utworzenie kodu nie powiodło się: %s", e)
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def use_card_key(self, key_code: str, user_id: int) -> Optional[int]:
        """Realizuje kod; zwraca liczbę pkt lub kody błędu ujemne."""
        conn = self.get_connection()
        cursor = conn.cursor(DictCursor)
        ...
        try:
            ...
            # Czy użytkownik już użył tego kodu
            cursor.execute(
            ...
            logger.error("Użycie kodu nie powiodło się: %s", e)
            conn.rollback()
            return None
        finally:
            cursor.close()
    ...
    def get_all_card_keys(self, created_by: Optional[int] = None) -> List[Dict]:
        """Wszystkie kody (opcjonalnie po twórcy)."""
        conn = self.get_connection()
        cursor = conn.cursor(DictCursor)
...
# Alias zgodny ze starszym kodem (SQLite)
